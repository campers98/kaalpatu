...
from StoneXMusic import YouTube
...
from StoneXMusic.utils.formatters import seconds_to_min
...
from StoneXMusic.utils.stream.autoclear import auto_clean
from StoneXMusic.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Stone.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await Stone.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await Stone.mute_stream(chat_id)
        ...
        await Stone.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Stone.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Stone.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await Stone.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
